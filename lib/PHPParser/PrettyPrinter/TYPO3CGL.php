...
class PHPParser_PrettyPrinter_TYPO3CGL extends PHPParser_PrettyPrinterAbstract
{
    ...
    public function pExpr_Array(PHPParser_Node_Expr_Array $node) {
        return 'array(' . $this->pCommaSeparated($node->items, $node->itemsHaveLineBreaks()) . ')';
    }
    ...
    public function pStmt_Namespace(PHPParser_Node_Stmt_Namespace $node) {
        return 'namespace' . (null !== $node->name ? ' ' . $this->p($node->name) : '')
             . ';' . PHP_EOL . PHP_EOL . $this->pStmts($node->stmts, false) . PHP_EOL . '';
    }
    ...
    public function pStmt_Class(PHPParser_Node_Stmt_Class $node) {
        return $this->pModifiers($node->type)
             ...
             . ' {' . PHP_EOL . PHP_EOL . $this->pStmts($node->stmts) . PHP_EOL . '}' . PHP_EOL;
    }
    ...
    public function pStmt_TraitUse(PHPParser_Node_Stmt_TraitUse $node) {
        return 'use ' . $this->pCommaSeparated($node->traits)
             . (empty($node->adaptations)
                ? ';'
                : ' {' . PHP_EOL . $this->pStmts($node->adaptations) . PHP_EOL . '}');
    }
    ...
    public function pStmt_Property(PHPParser_Node_Stmt_Property $node) {
        return $this->pModifiers($node->type) . $this->pCommaSeparated($node->props) . ';' . PHP_EOL;
    }
    ...
    public function pStmt_Switch(PHPParser_Node_Stmt_Switch $node) {
        return 'switch (' . $this->p($node->cond) . ') {'
             . PHP_EOL . $this->pImplode($node->cases) . '}';
    }
    ...
    public function pStmt_InlineHTML(PHPParser_Node_Stmt_InlineHTML $node) {
        return '?>' . $this->pSafe(
            (PHP_EOL === $node->value[0] || "\r" === $node->value[0] ? PHP_EOL : '')
            . $node->value
        ) . '<?php ';
    }
